...
export type TreatmentProfitability = {
  summary: { sessionCount: number; recognizedRevenue: string; hppAmount: string; grossProfit: string; grossMarginPercent: string };
  sessions: Array<{ id: string; sessionCode: string; completedAt: string; recognizedRevenue: string; hppAmount: string; grossProfit: string; completionJournalEntryId: string; materialPostingId: string | null; branch: { branchCode: string; name: string } }>;
};
...
export const revenueApi = {
  ...
  profitability: async () => unwrap<TreatmentProfitability>(await api.get('/revenue/profitability')),
