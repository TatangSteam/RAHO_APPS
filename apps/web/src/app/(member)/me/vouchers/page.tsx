        ...
        <div>
          ...
          {others.length > 0 && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
                {others.map(pkg => <PackageCard key={pkg.id} pkg={pkg} />)}
              </div>
            </div>
          )}
        </div>
...
function PackageCard({ pkg }: { pkg: MemberPackage }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ borderTop: '1px solid var(--surface-border)', paddingTop: 10, display: 'flex', justifyContent: 'space-between' }}>
