    ...
    {loading ? <p className="text-sm text-neutral-500">Memuat ledger…</p> : <>
      {tab === 'accounts' && <section className="space-y-4">
        ...
        <div className="max-h-[640px] overflow-auto rounded-2xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900"><table className="w-full min-w-[760px] text-sm">
          ...
          <tbody>{filteredAccounts.map((account) => <tr key={account.id} className="group border-t border-neutral-100 transition-colors hover:bg-blue-50/60 dark:border-neutral-800 dark:hover:bg-blue-950/20">
            ...
            {canManageAccounts && <Td><div className="flex gap-2"><SmallButton onClick={async () => { const name = window.prompt('Nama akun', account.name); if (!name || name === account.name) return; try { await accountingApi.updateAccount(account.id, { name }); showToast.success('Akun diperbarui.'); await reload(); } catch (error: any) { showToast.error(error.response?.data?.error?.message || 'Gagal memperbarui akun.'); } }}>Edit</SmallButton><SmallButton onClick={async () => { if (!window.confirm(`Hapus akun ${account.code} — ${account.name}?`)) return; try { await accountingApi.deleteAccount(account.id); showToast.success('Akun dihapus.'); await reload(); } catch (error: any) { showToast.error(error.response?.data?.error?.message || 'Akun tidak dapat dihapus.'); } }}>Hapus</SmallButton></div></Td>}
          </tr>)}</tbody>
      ...
      {tab === 'periods' && <section className="space-y-4">
        ...
        <div className="grid gap-3 md:grid-cols-2">{periods.map((period) => <div key={period.id} className="rounded-xl border bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900">
          ...
          {canManagePeriods && <div className="mt-3 flex flex-wrap gap-2">{period.status === 'OPEN' && <><SmallButton onClick={async () => { const name = window.prompt('Nama periode', period.name); if (!name || name === period.name) return; try { await accountingApi.updatePeriod(period.id, { name }); showToast.success('Periode diperbarui.'); await reload(); } catch (error: any) { showToast.error(error.response?.data?.error?.message || 'Periode tidak dapat diedit.'); } }}>Edit</SmallButton><SmallButton onClick={async () => { if (!window.confirm(`Hapus periode ${period.name}?`)) return; try { await accountingApi.deletePeriod(period.id); showToast.success('Periode dihapus.'); await reload(); } catch (error: any) { showToast.error(error.response?.data?.error?.message || 'Periode tidak dapat dihapus.'); } }}>Hapus</SmallButton><SmallButton onClick={async () => { await accountingApi.setPeriodStatus(period.id, 'CLOSED', 'Ditutup melalui halaman accounting'); await reload(); }}>Tutup</SmallButton></>}{period.status === 'CLOSED' && <><SmallButton onClick={async () => { await accountingApi.setPeriodStatus(period.id, 'OPEN', 'Dibuka kembali melalui halaman accounting'); await reload(); }}>Buka</SmallButton><SmallButton onClick={async () => { await accountingApi.setPeriodStatus(period.id, 'LOCKED', 'Dikunci permanen melalui halaman accounting'); await reload(); }}>Lock</SmallButton></>}</div>}
        </div>)}</div>
      </section>}
