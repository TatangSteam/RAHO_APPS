...
export default function StockOpnamePage() {
  ...
  const reload = async () => { setLoading(true); try { setRows((await stockOpnameApi.list()).data); } catch (error) {
      assertCaughtError(error); showToast.error(error.response?.data?.error?.message || 'Gagal memuat stock opname.'); } finally { setLoading(false); } };
  useEffect(() => { void reload(); }, []);
  const act = async (operation: () => Promise<unknown>, message: string) => { try { await operation(); showToast.success(message); await reload(); } catch (error) {
