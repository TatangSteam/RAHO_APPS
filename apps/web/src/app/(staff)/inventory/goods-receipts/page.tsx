  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setPurchaseOrders((poResponse.data?.data?.data || []).filter(
        (po: PurchaseOrderInventory) => po.status === 'APPROVED' || po.status === 'PARTIALLY_RECEIVED',
      ));
