...
export function UnfinishedSessionReminderModal() {
  ...
  const loadReminders = useCallback(async () => {
    if (!enabled || !user?.userId || requestInFlight.current) return;

    const reminderSessionKey = `${REMINDER_SESSION_KEY_PREFIX}:${user.userId}:${user.role}`;
    ...
    try {
      ...
      setData(response);
      // A failed request must not suppress the reminder for the rest of the
      // browser session. Mark it as shown only after data was loaded.
  ...
  const title = 'Sesi terapi perlu dilengkapi';
  const subtitle = 'Semua bagian yang belum terisi ditampilkan di sini. Isi bagian yang menjadi akses Anda atau koordinasikan dengan petugas terkait.';
      ...
      <div className="mb-4 flex items-center justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-500/25 dark:bg-amber-500/10">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-amber-700/90 dark:text-amber-200/80">
            Merah dapat Anda isi. Abu-abu perlu dikoordinasikan dengan petugas yang berwenang.
          </p>
        </div>
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {item.missingSteps.map((step) => (
                <span
                  key={step.key}
                  className={step.actionable
                    ? 'inline-flex items-center gap-1 rounded-full bg-red-50 px-2.5 py-1 text-[11px] font-semibold text-red-700 dark:bg-red-500/10 dark:text-red-300'
                    : 'inline-flex items-center gap-1 rounded-full bg-neutral-100 px-2.5 py-1 text-[11px] font-semibold text-neutral-600 dark:bg-neutral-800 dark:text-neutral-300'}
                >
                  {step.actionable
                    ? <PencilLine className="h-3 w-3" />
                    : <Users className="h-3 w-3" />}
                  {step.actionable ? 'Isi' : 'Koordinasikan'}: {step.label}
                </span>
              ))}
            </div>
          </button>
