...
test.describe('Logistics dashboard valuation', () => {
  test('labels incomplete valuation and remains usable on mobile', async ({ page, request }) => {
    ...
    const totalAsset = page.getByText('Total nilai persediaan', { exact: true }).locator('..').locator('strong');
  ...
  test('values existing stock from a short form without changing quantity', async ({ page }) => {
    ...
    await expect(page.getByRole('cell', { name: 'Tidak ada stok yang perlu HPP.' })).toBeVisible();
    expect(savedPayload).toMatchObject({ adjustment: 0, unitCost: '15000', valuationDocumentReference: 'OPENING-2026-001', reasonCode: 'LEGACY_OPENING_VALUATION', stockLocationId: 'location-1', notes: 'HPP stok awal berdasarkan dokumen OPENING-2026-001' });
  });
  ...
  test('shows a searched SKU with no cost layer and lets Super Admin value it', async ({ page }) => {
    ...
    const noLayer = {
      ...
      data: [{ ...mockValuation.data[0], onHandQty: '72', pendingValuationQty: '0', quantityReconciled: false, costLayers: [], masterProduct: { ...mockValuation.data[0].masterProduct, sku: 'PRD-ANN-BRU-001', name: 'Air Nano Biru 600ml', baseUnit: 'Botol', unit: 'Botol' } }],
    ...
    await expect(dialog.getByText('Jumlah stok tidak berubah.')).toBeVisible();
    await dialog.getByRole('button', { name: 'Simpan HPP' }).click();
    await expect(dialog).not.toBeVisible();
    await expect(page.getByText('Stok siap dijual: PRD-ANN-BRU-001')).toBeVisible();
  ...
  test('explains when the SKU is not assigned to the selected branch', async ({ page }) => {
    ...
    await expect(page.getByText('Pilih cabang transaksi yang benar.')).toBeVisible();
  ...
  test('lets Super Admin fill HPP when legacy stock has no assigned location', async ({ page }) => {
    ...
    await page.route('**/api/v1/**', async (route) => {
      ...
      else if (url.includes('/inventory/reports/valuation')) data = url.includes('search=PRD-ANN-BRU-001') ? unlisted : { ...mockValuation, data: [], meta: { ...mockValuation.meta, total: 0, totalPages: 0 } };
      else if (url.includes('/inventory/items/item-1/adjust-stock') && route.request().method() === 'PATCH') {
        savedPayload = route.request().postDataJSON();
        data = { valuation: { quantityValued: '72', unitCost: '12000', totalValue: '864000' } };
      }
