...
const base = { contractVersion: 'rain.v1', success: true, source: 'erp', isDemo: false, asOf: '2026-09-16T14:30:00+07:00', user: { id: userId }, mode: 'openclaw', intent: 'agent' };
...
test('RAIN is accessible under Extra for MSO, answers ERP results and stays usable on mobile', async ({ page }) => {
  ...
  await page.route('**/api/v1/**', async (route) => {
    if (route.request().url().includes('/ai/chat')) {
      ...
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ ...base, conversationId: payload.conversationId, reply: 'Hari ini ada 8 task, 5 selesai. Completion rate 71,4%.', context: null, data: null }) });
  ...
  expect(payload).toMatchObject({ message: 'Kinerja hari ini', conversationId: expect.any(String) });
...
test('RAIN keeps one opaque conversation id and never sends another user identity', async ({ page }) => {
  await login(page);
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({
      ...base, conversationId: payloads[0].conversationId, reply: first ? 'Ada 11 task. Ketik lanjut.' : 'Hasil berikutnya.', context: null,
  ...
  await page.getByRole('textbox', { name: 'Pertanyaan untuk RAIN' }).fill('lanjut');
  await page.getByRole('button', { name: 'Kirim', exact: true }).click();
  await expect(page.getByText('Cek laporan', { exact: true })).toBeVisible();
  expect(payloads[1]).toMatchObject({ message: 'lanjut', conversationId: payloads[0].conversationId });
  expect(payloads[0]).not.toHaveProperty('userId');
