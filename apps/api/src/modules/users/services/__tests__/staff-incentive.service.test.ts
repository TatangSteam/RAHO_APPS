...
describe('staff monthly incentive rules', () => {
  ...
  it('calculates all CHS target, paid, personal, and Team HO components without multiplying bonuses', () => {
    expect(calculateChsCoordinatorMonthlyIncentive(400, 110, {
      qualifiedHomecareTeams: 2,
      qualifiedBranches: 3,
      hoPaidInfusions: 20,
    })).toMatchObject({
      paidInfusionAmount: 400_000,
      homecareTeamTargetBonus: 1_000_000,
      branchTargetBonus: 750_000,
      hoInfusionAmount: 200_000,
      personalInfusionAmount: 1_100_000,
      personalTargetBonus: 2_000_000,
      totalAmount: 5_450_000,
    });
  });

  it('calculates Doctor Head components at their exact qualifier boundaries', () => {
    expect(calculateDoctorHeadMonthlyIncentive({
      qualifiedHomecareTeams: 2,
      qualifiedBranches: 1,
      homecareDoctorPaidInfusions: 80,
      partnershipTotalInfusions: 1_499,
      partnershipPaidInfusions: 1_000,
    })).toMatchObject({ partnershipTargetReached: false, partnershipAmount: 0, totalAmount: 1_900_000 });
    expect(calculateDoctorHeadMonthlyIncentive({
      qualifiedHomecareTeams: 2,
      qualifiedBranches: 1,
      homecareDoctorPaidInfusions: 80,
      partnershipTotalInfusions: 1_500,
      partnershipPaidInfusions: 1_000,
    })).toMatchObject({
      homecareTeamTargetBonus: 1_000_000,
      branchTargetBonus: 500_000,
      homecareDoctorAmount: 400_000,
      partnershipTargetReached: true,
      partnershipAmount: 2_000_000,
  ...
  it('excludes free, social, discount above 40%, unpaid, and refunded CHS infusions', () => {
    ...
    const { discountPercent: _discountPercent, ...paidEligibility } = eligible;
    expect(isPaidInfusion(paidEligibility)).toBe(true);
  });
  ...
  it('exports valid Excel sheets for Nakes, MSO, and CHS coordinators', async () => {
    ...
    expect(workbook.worksheets.map((sheet) => sheet.name)).toEqual(['Ringkasan', 'NAKES', 'MSO', 'KOORDINATOR CHS', 'DETAIL QUALIFIER CHS', 'DOKTER HEAD', 'DETAIL DOKTER HEAD']);
