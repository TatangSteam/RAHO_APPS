...
import { Prisma, PurchaseRequestStatus, SupplierStatus } from '@prisma/client';
...
export async function submitPurchaseRequest(userId: string, requestId: string) {
  ...
  const result = await prisma.purchaseRequest.update({ where: { id: requestId }, data: { status: 'SUBMITTED', submittedAt: new Date(), rejectionReason: null } });
...
export async function rejectPurchaseRequest(userId: string, requestId: string, reason: string) {
  ...
  if (row.status !== PurchaseRequestStatus.SUBMITTED) throw errors.conflict('PURCHASE_REQUEST_NOT_SUBMITTED', 'PR belum diajukan.');
  const result = await prisma.purchaseRequest.update({ where: { id: requestId }, data: { status: 'REJECTED', rejectionReason: reason, reviewedBy: userId, reviewedAt: new Date() } });
...
export async function postGoodsReceipt(userId: string, purchaseOrderId: string, input: CreateGoodsReceiptInput) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await tx.goodsReceipt.create({ data: { id: receiptId, receiptNumber, idempotencyKey: input.idempotencyKey, payloadHash, purchaseOrderId: po.id, branchId: po.branchId, receiptDate: input.receiptDate, totalValue, journalEntryId: posted.journal.id, evidenceReference: input.evidenceReference, createdBy: userId } });
