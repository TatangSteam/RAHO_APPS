...
import { profitabilityQuerySchema, revenueListQuerySchema, upsertRevenuePolicySchema } from './revenue.schema';
import { getTreatmentProfitability, listRevenueContracts, listRevenuePolicies, listTreatmentEvents, upsertRevenuePolicy } from './revenue.service';
...
export async function profitability(req: Request, res: Response, next: NextFunction) { try { sendSuccess(res, await getTreatmentProfitability(req.user.userId, profitabilityQuerySchema.parse(req.query))); } catch (error) { next(error); } }
