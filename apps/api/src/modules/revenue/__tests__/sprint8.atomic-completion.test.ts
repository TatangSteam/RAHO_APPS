...
describe('Sprint 8 atomic treatment completion', () => {
  ...
  it('mengikat session, stock, revenue, HPP dan journal dalam satu Prisma transaction', () => {
    ...
    expect(completion).toContain('status: IntegrationEventStatus.PENDING');
    expect(revenue).toContain('postTreatmentCompletionDerivedJournal');
    expect(revenue).toContain("data: { status: 'POSTED', journalEntryId");
    expect(revenue).toContain("data: { status: 'PROCESSED', processedAt");
  });
});
